    ...
    return (
        <nav className="py-6 px-10 w-full bg-black">
            <div className="flex justify-between items-center container mx-auto bg-black">
                <div><h1 className="max-[640px]:text-sm text-lg text-zinc-50 font-bold"><Link to="/">Books</Link></h1></div>
                <>{showError ? <div><h1 className="max-[640px]:text-sm text-lg text-red-600 font-bold ml-5 mr-5">Signin Failure: {errorMessage}, Try again</h1></div> : ''}</>
                <>{signedIn ? <div><h1 className="max-[640px]:text-sm text-center text-lg text-zinc-50 font-bold">Hello {userName}</h1></div> : ''}</>
                <button onClick={()=>authChange({authChange,authDataInfo})} className="max-[640px]:text-sm text-lg text-zinc-50 font-bold">
